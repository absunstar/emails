...
Manual deletion must call the shared service with an authenticated administrator context. Automatic cleanup is owned by the file store and only runs when the message count exceeds `EMAIL_MAX_MESSAGES` (default 10,000), skipping VIP/company mail.
